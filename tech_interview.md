...
HashMap的实现大概率会考，看put函数源码即可。

比较偏的考点，将链表转红黑树的treefy阈值为什么是8，网上的说法主要是8的时候概率比较小，个人感觉这值是7是9对效率的影响也不大。

## ConcurrentHashMap

大体的逻辑和HashMap差不多，数组存链表的头结点，每个链表代表一个hash桶，当链表长度达到一定程度时（默认8），会将链表转换为红黑树

HashTable是直接在put上加的大锁，一般不考虑。

## SkipList 跳表

在《算法导论》里并没有介绍， 但由于某些方面的优点在Redis里使用了。
...
跳表并没有维护一个绝对均匀的索引，而是通过随机概率的形式保证上一级的索引的链表长度是当前级链表长度的一半。因此对比红黑树，插入和删除过程更为直接和高效，区间查询的速度快。
...
网络技术大多只考三次握手四次挥手，稍微了解一下细节就差不多。java后台开发可能会考http。
...
GC算法和垃圾收集器（通常必考，可深可浅，最好知道优缺点，有些博客的存在一定的误导）
...
G1内存划分相对比较特殊。把G1读为G one可能会有气势加成。
...
jdk8 默认使用G1垃圾收集器，通常JVM将堆分成了 二个大区  Young 和 Old（新生代和老年代），而Young区分为 Eden、Servivor1、Servivor2已经不怎么合适了，按上面的G1的说法比较好。
...
悲观锁，某个线程访问资源时，会拒绝其它线程访问资源。也成独占锁、互斥锁。如synchronized。

乐观锁，假设通常不会发声冲突，发生冲突时进行重试。如CAS。

### CAS

必考

CAS是乐观锁的一种实现方式，在修改某一个变量时，只能由一个线程修改。

底层的实现调用了native实现了unsafe类，在CPU的底层对要访问的内存区域做了加锁，在原子操作中比较是否为先前的值，如果是则替换为新的值。

相对于Synchronize是一种更轻量级的锁
...
面试题（快手）：为什么阿里的java开发手册不建议使用原生线程池？
...
- java泛型的实现机制和优缺点（字节跳动）
- java集合类

## 闭包

字节跳动问的一个问题，查了些东西感觉在java里用这个概念不太合适，估计不怎么会考。

这是一个函数式编程里的概念，主要是函数内部使用了函数外的变量，然后由外部函数形成闭包。

用在java上主要体现为使用内部类，内部类能够直接访问外部类的成员。

使用内部类相对于定义独立的类，最大的优点在于能够访问外部类的所有成员。
...
主考双重判断的单例，然后扩展到volatile关键字和synchronize加锁。
...
通过消息队列和异步处理的方式，保证后端系统不用直接处理大量的流量。秒杀的后端访问有读多写少的特点，因此可以考虑用redis做读写的平台
